...
class ThanhToanController extends Controller
{
    ...
    public function kiemTraTrangThai(Request $request)
    {
        try {
            ...
            $tenPhutTruoc = $now->copy()->subMinutes(50);

            $url = 'https://my.sepay.vn/userapi/transactions/list';

            $response = Http::withHeaders([
                'Authorization' => 'Bearer 464COBT8L1ZNKM2A079JMPPG7KBE5YSSIWHAJUKMXVRE2WJD3GAQVCUPUEXCHBDW',
            ...
            foreach ($data['transactions'] as $giaoDich) {
                if (
                    !isset($giaoDich['transaction_date'], $giaoDich['transaction_content'], $giaoDich['amount_in'])
                ) {
                    Log::warning("Giao dịch thiếu thông tin: " . json_encode($giaoDich));
                    continue;
                }
                ...
                if ($thoiGianGiaoDich->between($tenPhutTruoc, $now)) {
                    ...
                    if (strpos($noiDungGiaoDich, $noiDungCanTim) !== false) {
                        ...
                        if (!$datVe) {
                            ...
                            return response()->json([
                                'success' => false,
                                'message' => 'Không tìm thấy bản ghi đặt vé.'
                            ], 404);
                        }

                        if (!is_numeric($datVe->tong_tien)) {
                            Log::error("Tổng tiền không hợp lệ: " . $datVe->tong_tien);
                            return response()->json([
                                'success' => false,
                                'message' => 'Tổng tiền hệ thống không hợp lệ.'
                            ], 500);
                        }
                        ...
                        $datVe->ngay_thanh_toan = Carbon::parse($giaoDich['transaction_date'])->timezone('Asia/Ho_Chi_Minh');
