...
@media (max-width: 768px) {
    .ticket-container {
        grid-template-columns: 1fr;
>>>>>>> 2400f7b01d57451553fc174b0bba0f921223899d
    }
...
{{-- <div id="invoice-section" class="p-4">
        ...
        @foreach ($datVe->gheNgois as $ghe)
            <div class="ticket">
                ...
                <p class="ticket-line"><strong>Phim:</strong> {{ $datVe->suatChieu?->phim?->ten_phim ?? 'Không xác định' }}
</p>
<p class="ticket-line"><strong>Thời gian:</strong>
    {{ optional($datVe->suatChieu)->ngay_chieu ? \Carbon\Carbon::parse($datVe->suatChieu->ngay_chieu)->format('d/m/Y') : '---' }}
    - {{ $datVe->suatChieu?->bat_dau ?? '---' }}
</p>
<p class="ticket-line"><strong>Rạp:</strong>
    {{ $datVe->suatChieu?->phongChieu?->rapPhim?->ten_rap ?? 'Không xác định' }}
</p>
<p class="ticket-line"><strong>Chi nhánh:</strong>
...
<div class="ticket">
    ...
    <p class="ticket-line font-bold"><strong>Tổng tiền đồ ăn:</strong>
        {{ number_format($tongTienCombo + $tongTienDoAn, 0, ',', '.') }} VND
    </p>
    <div class="barcode">
...
<div class="ticket">
    ...
    <p class="ticket-line font-bold"><strong>Tổng thanh toán:</strong>
        {{ number_format($tongThanhTien, 0, ',', '.') }} VND
    </p>
</div>
</div> --}}
