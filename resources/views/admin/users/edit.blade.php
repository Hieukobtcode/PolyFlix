...
@section('breadcrumb', 'Chỉnh sửa Người dùng')
    ...
    <div class="container-fluid">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold">Chỉnh sửa người dùng: {{ $user->name }}</h5>
                        ...
                        <div class="col-md-6">
                                ...
                                <select id="vai_tro_id" name="vai_tro_id" class="form-select @error('vai_tro_id') is-invalid @enderror">
                                    ...
                                    @foreach($vaiTros as $vaiTro)
                                        <option value="{{ $vaiTro->id }}"
                                            {{ old('vai_tro_id', $user->vai_tro_id) == $vaiTro->id ? 'selected' : '' }}>
                                            {{ $vaiTro->ten }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select id="trang_thai" name="trang_thai" class="form-select @error('trang_thai') is-invalid @enderror">
                                    ...
                                    <option value="active" {{ old('trang_thai', $user->trang_thai) === 'active' ? 'selected' : '' }}>Active</option>
                                    <option value="block" {{ old('trang_thai', $user->trang_thai) === 'block' ? 'selected' : '' }}>Block</option>
                                </select>
