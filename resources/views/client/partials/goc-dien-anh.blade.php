...
<div class="new">
    ...
    <div class="list">
        ...
        <p>
            <a class="tab-item" data-tab="blog">Blog điện ảnh</a>
        </p>
    </div>
</div>
        ...
        @foreach ($allPhims as $index => $movie)
            <div class="slide" style="{{ $index === 0 ? '' : 'display:none;' }}">
                <div class="khung-binh-luan">
                    ...
                    {{-- Bình luận --}}
                    <div class="binh-luan">
                        <h4 class="ten-phim">{{ $movie->ten_phim }}</h4>
                        @forelse ($movie->comments as $binhLuan)
                            @php
                                $userRating = $binhLuan->user
                                    ? $binhLuan->user->ratings()->where('phim_id', $binhLuan->phim_id)->first()
                                    : null;
                            @endphp
