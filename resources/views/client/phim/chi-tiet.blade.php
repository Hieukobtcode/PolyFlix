...
@section('content')
    ...
    <style>
        ...
        .sidebar-movie-title {
            ...
            max-width: 200px;
            min-height: 2.56em;
            /* Để các item cao đều nhau nếu muốn */
        }
        ...
        .lich-chieu-rap-box {
            background: rgba(16, 21, 46, 0.92) !important;
            border-radius: 13px;
    ...
    <div class="container py-3">
        ...
        <div class="movie-detail-wrapper"
            style="background:rgba(35,40,74,0.90); border-radius:16px; box-shadow:0 4px 22px #0d10205e; padding: 28px 22px;">
            <div class="row g-4">
                <!-- Card phim chi tiết -->
                <div class="col-lg-9">
                    <div class="movie-detail-card" style="background:transparent; box-shadow:none; border-radius:0;">
                        <!-- Poster & Trailer -->
                        ...
                        <div class="movie-info">
                            <div class="movie-title" style="display: flex; align-items: center; gap: 10px;">
                                {{ $phim->ten_phim }}
                                @if ($phim->do_tuoi)
                            ...
                            <div class="movie-info-row">
                                ...
                                <div class="movie-info-col">
                                    <span class="movie-label">Diễn viên:</span>
        ...
        <div class="col-lg-3">
            <div class="sidebar-nowshowing" style="background:transparent; box-shadow:none; border-radius:0;">
                <h4 style="">Phim Đang Chiếu</h4>
                @foreach ($phimDangChieu as $p)
                    <div class="sidebar-movie">
                        <a href="{{ route('phim.chi-tiet', $p->id) }}">
                            <img src="{{ asset('storage/' . $p->poster) }}" alt="{{ $p->ten_phim }}">
                        </a>
                        <a class="sidebar-movie-title" href="{{ route('phim.chi-tiet', $p->id) }}">
                            {{ Str::limit($p->ten_phim, 18) }}
                        </a>
                    </div>
                @endforeach
            </div>
    ...
    <div id="trailerPopup"
        style="display:none; position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#000;padding:10px 0 0 0;border-radius:12px;z-index:999;">
        <iframe id="trailerIframe" width="760" height="430" frameborder="0" allowfullscreen></iframe>
        <button onclick="hideTrailer()"
            style="display:block;margin:12px auto 10px auto;background:#febe27;color:#181d2f;font-weight:bold;padding:8px 32px;border:none;border-radius:22px;">X</button>
    </div>
            ...
            function loadLichChieu() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (!data.html || data.html.trim() === '' || data.html.includes(
                                'Không có suất chiếu')) {
